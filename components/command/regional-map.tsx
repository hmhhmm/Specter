...
export function RegionalMap() {
  ...
  return (
    <div className="relative w-full h-full bg-zinc-100 dark:bg-zinc-950/60 rounded-lg border border-zinc-200 dark:border-zinc-800/50 overflow-hidden transition-colors duration-300">
      {/* World Map Background */}
      <svg viewBox="0 0 100 80" className="w-full h-full opacity-10 absolute inset-0">
        <defs>
          <pattern id="grid" width="10" height="10" patternUnits="userSpaceOnUse">
            <path d="M 10 0 L 0 0 0 10" fill="none" stroke="currentColor" strokeWidth="0.5" className="text-zinc-300 dark:text-zinc-700"/>
          </pattern>
        </defs>
        ...
        {regions.map((region, i) => (
          ...
          >
            <div className="flex flex-col items-center gap-1">
              ...
              {/* Region Label */}
              <div className="px-2 py-0.5 rounded bg-zinc-200/90 dark:bg-zinc-900/80 border border-zinc-300 dark:border-zinc-700/50 backdrop-blur-sm transition-colors duration-300">
                <div className="text-[7px] font-medium text-zinc-700 dark:text-zinc-400 whitespace-nowrap">{region.name}</div>
                {region.issues > 0 && (
                  <div className="text-[6px] text-amber-600 dark:text-amber-400 text-center">{region.issues} issues</div>
                )}
              </div>
            </div>
          </div>
