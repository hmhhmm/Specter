...
export function RunCard({ run, onExpand, isExpanded }: RunCardProps) {
  ...
  return (
    <div className={cn(
      ...
      config.color,
      isExpanded ? "bg-zinc-900/60" : "bg-zinc-900/40 hover:bg-zinc-900/60"
    )}>
      ...
      {isExpanded && (
        <div className="border-t border-white/10">
          ...
          {run.incidents.map((incident: any, idx: number) => (
            <div
              key={incident.id || idx}
              className="rounded-lg bg-white/5 border border-white/10 overflow-hidden hover:bg-white/[0.07] transition-all"
            >
              ...
              <div className="p-3.5">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    {incident.device && (
                      <div className="px-2 py-0.5 rounded bg-zinc-800/50 border border-zinc-700/50 text-[11px] text-zinc-400 font-medium">
                        {incident.device}
                      </div>
                    )}
                  </div>
                ...
                {(incident.screenshot_before || incident.screenshot_after) && (
                  <div className="mt-4">
                    <button
                      ...
                      className="text-xs font-medium text-emerald-400 hover:text-emerald-300 transition-colors mb-2"
                    >
                      🔽 Show Evidence
                    </button>
